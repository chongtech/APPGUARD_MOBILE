...
function StatusBadge({ status }: { status: VisitStatus }) {
  ...
  return (
    <View style={[styles.badge, { backgroundColor: c.bg }]}>
      <ThemedText style={[styles.badgeText, { color: c.text }]}>
        {status}
      </ThemedText>
    </View>
  );
...
export default function DailyListScreen() {
  ...
  const handleCheckout = (visit: Visit) => {
    Alert.alert(
      "Confirmar saída?",
      `Registar saída de ${visit.visitor_name}?`,
      [
  ...
  const handleCall = (visit: Visit) => {
    ...
    Alert.alert(`Ligar para ${visit.visitor_phone}?`, "", [
      ...
      {
        text: "Ligar",
        onPress: () => Linking.openURL(`tel:${visit.visitor_phone}`),
      },
    ]);
  };
      ...
      <View style={styles.cardHeader}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <ThemedText type="small" style={{ color: theme.textSecondary }}>
            {new Date(v.check_in_at).toLocaleTimeString("pt-PT", {
              hour: "2-digit",
              minute: "2-digit",
            })}
  ...
  return (
    <ThemedView style={styles.container}>
      ...
      <Modal
        ...
        transparent
        onRequestClose={() =>
          setHistoryModal((s) => ({ ...s, visible: false }))
        }
      >
              ...
              <View>
                ...
                {historyModal.visit && (
                  <ThemedText type="small" style={{ color: "#94A3B8" }}>
                    {historyModal.visit.visitor_name} •{" "}
                    {historyModal.visit.visit_type}
                  </ThemedText>
                )}
              </View>
              <Pressable
