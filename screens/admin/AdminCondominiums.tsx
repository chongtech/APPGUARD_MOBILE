...
export default function AdminCondominiums() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setItems(await api.adminGetAllCondominiums());
    } catch (loadError) {
      logger.warn(LogCategory.UI, "AdminCondominiums: load failed", {
        ...
        <View style={styles.overlay}>
          ...
          >
            <View style={styles.sheetHeader}>
              <ThemedText type="h3">
                {editing ? "Editar" : "Novo Condomínio"}
              </ThemedText>
