...
export default function AdminStaff() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setItems(await api.adminGetAllStaff());
    } catch (loadError) {
      logger.warn(LogCategory.UI, "AdminStaff: load failed", {
  ...
  const openEdit = (s: Staff) => {
    ...
    setRole(s.role);
    setPin("");
    setCondoId(String(s.condominium_id));
    setModalOpen(true);
  };

  const handleSave = async () => {
    if (!firstName.trim())
      return Alert.alert("Erro", "Primeiro nome obrigatório.");
    setSaving(true);
    try {
      if (editing) {
          ...
          renderItem={({ item: s }) => (
            ...
            >
              <View style={styles.cardMain}>
                ...
                >
                  <ThemedText
                    style={{ fontWeight: "800", color: BrandColors.primary }}
                  >
                ...
                <View
                  style={[
                    styles.badge,
                    {
                      backgroundColor: (ROLE_COLOR[s.role] ?? "#64748B") + "20",
                    },
                  ]}
                >
                  <ThemedText
        ...
        <View style={styles.overlay}>
          ...
          >
            <View style={styles.sheetHeader}>
              <ThemedText type="h3">
                {editing ? "Editar Staff" : "Novo Staff"}
              </ThemedText>
            ...
            >
              {(
                [
                  ["Primeiro Nome *", firstName, setFirstName],
                  ["Apelido", lastName, setLastName],
                    ...
                    <Pressable
                      ...
                      style={[
                        styles.chip,
                        {
                          ...
                          borderColor:
                            role === r ? BrandColors.primary : theme.border,
                        },
                      ]}
                      onPress={() => setRole(r)}
                    >
