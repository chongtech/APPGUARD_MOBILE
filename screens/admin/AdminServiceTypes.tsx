...
export default function AdminServiceTypes() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setItems(await api.adminGetAllServiceTypes());
    } catch (loadError) {
      logger.warn(LogCategory.UI, "AdminServiceTypes: load failed", {
  ...
  const openEdit = (s: ServiceTypeConfig) => {
    setEditing(s);
    setName(s.name);
    setModalOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing)
        await api.adminUpdateServiceType(editing.id, { name: name.trim() });
  ...
  const handleDelete = (s: ServiceTypeConfig) => {
    Alert.alert("Eliminar", `Eliminar "${s.name}"?`, [
      { text: "Cancelar", style: "cancel" },
      {
        ...
        style: "destructive",
        onPress: async () => {
          await api.adminDeleteServiceType(s.id);
          load();
        },
      },
    ]);
