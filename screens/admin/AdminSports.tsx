...
export default function AdminSports() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setItems(await api.adminGetAllSports());
    } catch (loadError) {
      logger.warn(LogCategory.UI, "AdminSports: load failed", {
  ...
  const handleDelete = (s: Sport) => {
    Alert.alert("Eliminar", `Eliminar "${s.name}"?`, [
      { text: "Cancelar", style: "cancel" },
      {
        ...
        style: "destructive",
        onPress: async () => {
          await api.adminDeleteSport(s.id);
          load();
        },
      },
    ]);
              ...
              <View style={styles.cardRow}>
                ...
                >
                  <ThemedText
                    type="small"
                    style={{
                      color: s.status === "ACTIVE" ? "#065F46" : "#64748B",
                      fontWeight: "700",
                    }}
                  >
                    {s.status ?? "—"}
            ...
            >
              {(
                [
                  ...
                  ["Descrição", desc, setDesc, "default"],
                  ...
                  {(["ACTIVE", "INACTIVE"] as Status[]).map((st) => (
                    <Pressable
                      ...
                      style={[
                        styles.chip,
                        {
                          ...
                          borderColor:
                            status === st ? BrandColors.primary : theme.border,
                        },
                      ]}
                    ...
                    >
                      <ThemedText
                        type="small"
                        style={{
                          color: status === st ? "#fff" : theme.text,
                          fontWeight: "700",
                        }}
                      >
                        {st}
