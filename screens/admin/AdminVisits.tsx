...
export default function AdminVisits() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setItems(await api.adminGetAllVisits());
    } catch (loadError) {
      logger.warn(LogCategory.UI, "AdminVisits: load failed", {
    ...
    Alert.alert(v.visitor_name, "Alterar estado", [
      {
        text: "Aprovar",
        onPress: async () => {
          await api.adminUpdateVisitStatus(v.id, VisitStatus.APPROVED);
          load();
        },
      },
      {
        ...
        style: "destructive",
        onPress: async () => {
          await api.adminUpdateVisitStatus(v.id, VisitStatus.DENIED);
          load();
        },
      },
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item: v }) => {
            ...
            const dt = new Date(v.check_in_at).toLocaleString("pt-PT", {
              day: "2-digit",
              month: "short",
              hour: "2-digit",
              minute: "2-digit",
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    {v.unit_block && (
                      <ThemedText
                        ...
                        style={{ color: theme.textSecondary }}
                      >
                        Bloco {v.unit_block} – {v.unit_number}
                      </ThemedText>
                    )}
                  </View>
