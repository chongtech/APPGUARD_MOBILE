...
export default function AdminSubscriptions() {
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      setItems(await api.adminGetCondominiumSubscriptions());
    } catch (loadError) {
      logger.warn(LogCategory.UI, "AdminSubscriptions: load failed", {
  ...
  const openActions = (sub: CondominiumSubscription) => {
    Alert.alert(
      sub.condominium_name ?? `Condo #${sub.condominium_id}`,
      "Alterar estado:",
      [
        {
          text: "Ativar",
          onPress: async () => {
            await api.adminUpdateSubscriptionStatus(sub.id, "ACTIVE");
            load();
          },
        },
        {
          text: "Desativar",
          onPress: async () => {
            await api.adminUpdateSubscriptionStatus(sub.id, "INACTIVE");
            load();
          },
        },
        {
          text: "Trial",
          onPress: async () => {
            await api.adminUpdateSubscriptionStatus(sub.id, "TRIAL");
            load();
          },
        },
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item: sub }) => {
            ...
            const pc = sub.payment_status
              ? (PAYMENT_COLOR[sub.payment_status] ?? null)
              : null;
            return (
              <Pressable
                style={[
                ...
                <View style={styles.cardRow}>
                  ...
                  <View style={{ flex: 1, marginLeft: Spacing.md }}>
                    ...
                    {!!sub.months_in_arrears && sub.months_in_arrears > 0 && (
                      <ThemedText type="small" style={{ color: "#EF4444" }}>
                        {sub.months_in_arrears}{" "}
                        {sub.months_in_arrears === 1 ? "mês" : "meses"} em
                        atraso
                      </ThemedText>
                    )}
                  </View>
                  <View style={{ gap: 4, alignItems: "flex-end" }}>
                    <View style={[styles.badge, { backgroundColor: sc.bg }]}>
                      <ThemedText
                        type="small"
                        style={{ color: sc.text, fontWeight: "700" }}
                      >
                        {sub.status}
                      </ThemedText>
                    </View>
                    {pc && (
                      <View style={[styles.badge, { backgroundColor: pc.bg }]}>
                        <ThemedText
                          type="small"
                          style={{ color: pc.text, fontWeight: "700" }}
                        >
                          {sub.payment_status}
                        </ThemedText>
                      </View>
