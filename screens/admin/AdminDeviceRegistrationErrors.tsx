...
export default function AdminDeviceRegistrationErrors() {
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try { setItems(await api.adminGetDeviceRegistrationErrors()); } catch (loadError) { logger.warn(LogCategory.UI, "AdminDeviceRegistrationErrors: load failed", { error: String(loadError) }); } finally { setLoading(false); setRefreshing(false); }
  }, []);
