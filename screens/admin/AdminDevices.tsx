...
export default function AdminDevices() {
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      setItems(await api.adminGetAllDevices());
    } catch (loadError) {
      logger.warn(LogCategory.UI, "AdminDevices: load failed", {
  ...
  const handleDecommission = (d: Device) => {
    Alert.alert(
      "Decomissionar",
      `Decomissionar "${d.device_name ?? d.device_identifier}"?`,
      [
        { text: "Cancelar", style: "cancel" },
        {
          ...
          style: "destructive",
          onPress: async () => {
            await api.adminDecommissionDevice(d.id ?? d.device_identifier);
            load();
          },
        },
      ],
  ...
  return (
    <ThemedView style={styles.container}>
      ...
      {loading ? (
        ...
        <FlatList
          data={filtered}
          keyExtractor={(d) => d.id ?? d.device_identifier}
          ...
          renderItem={({ item: d }) => {
            const sc =
              STATUS_COLOR[d.status ?? "INACTIVE"] ?? STATUS_COLOR.INACTIVE;
            const seen = d.last_seen_at
              ? new Date(d.last_seen_at).toLocaleString("pt-PT", {
                  day: "2-digit",
                  month: "short",
                  hour: "2-digit",
                  minute: "2-digit",
            ...
            return (
              <Pressable
                ...
                onPress={() =>
                  d.status !== "DECOMMISSIONED" && handleDecommission(d)
                }
              >
                <View style={styles.cardRow}>
                  ...
                  <View style={{ flex: 1, marginLeft: Spacing.md }}>
                    ...
                    >
                      Condo #{d.condominium_id ?? "—"} · Visto: {seen}
                    </ThemedText>
                  </View>
