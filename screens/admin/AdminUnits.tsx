...
export default function AdminUnits() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setItems(await api.adminGetAllUnits());
    } catch (loadError) {
      logger.warn(LogCategory.UI, "AdminUnits: load failed", {
    ...
    () =>
      items.filter(
        (u) =>
          ...
          u.code_block?.toLowerCase().includes(search.toLowerCase()),
      ),
    [items, search],
  );
  ...
  const openEdit = (u: Unit) => {
    setEditing(u);
    setNumber(u.number);
    setBlock(u.code_block ?? "");
    setCondoId(String(u.condominium_id));
    setModalOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing)
        await api.adminUpdateUnit(editing.id, {
          number: number.trim(),
          code_block: block.trim() || undefined,
        });
      else
          ...
          >
            ...
            >
              {[
                ...
                ["Bloco", block, setBlock],
                ...(!editing ? [["Condo ID *", condoId, setCondoId]] : []),
              ].map(([label, value, set]) => (
