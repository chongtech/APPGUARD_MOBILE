...
import { View, FlatList, Pressable, StyleSheet, ActivityIndicator, TextInput, RefreshControl } from "react-native";
...
export default function AdminAuditLogs() {
  ...
  const filtered = useMemo(() =>
    items.filter((l) => l.action.toLowerCase().includes(search.toLowerCase()) || (l.actor?.first_name ?? "").toLowerCase().includes(search.toLowerCase())),
    [items, search]);
  ...
  return (
    <ThemedView style={styles.container}>
      ...
      <View style={[styles.searchRow, { backgroundColor: theme.backgroundSecondary, borderColor: theme.border }]}>
        ...
        <TextInput style={{ flex: 1, color: theme.text }} placeholder="Pesquisar acção ou actor..." placeholderTextColor={theme.textSecondary} value={search} onChangeText={setSearch} />
      </View>
      {loading ? <View style={styles.center}><ActivityIndicator color={BrandColors.primary} /></View> : (
        <FlatList data={filtered} keyExtractor={(l) => String(l.id)} contentContainerStyle={{ padding: Spacing.md, gap: Spacing.sm }}
          refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); load(true); }} />}
          ListEmptyComponent={<View style={styles.center}><ThemedText style={{ color: theme.textSecondary }}>Sem logs</ThemedText></View>}
          renderItem={({ item: log }) => {
            const dt = new Date(log.created_at).toLocaleString("pt-PT", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" });
            return (
              <View style={[styles.card, { backgroundColor: theme.cardBackground, borderColor: theme.border }]}>
                <View style={styles.cardRow}>
                  <View style={{ flex: 1 }}>
                    <ThemedText type="h4">{log.action}</ThemedText>
                    {log.target_table && <View style={styles.tableBadge}><ThemedText type="small" style={{ color: "#1E40AF", fontWeight: "700" }}>{log.target_table}</ThemedText></View>}
                    {log.actor && <ThemedText type="small" style={{ color: theme.textSecondary }}>Por: {log.actor.first_name} {log.actor.last_name}</ThemedText>}
                  </View>
                  <ThemedText type="small" style={{ color: theme.textSecondary }}>{dt}</ThemedText>
                </View>
              </View>
            );
          }} />
...
const styles = StyleSheet.create({
  ...
  tableBadge: { alignSelf: "flex-start", paddingHorizontal: 8, paddingVertical: 2, borderRadius: 4, backgroundColor: "#DBEAFE", marginTop: 2 },
});
