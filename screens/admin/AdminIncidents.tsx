...
export default function AdminIncidents() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setItems(await api.adminGetAllIncidents());
    } catch (loadError) {
      logger.warn(LogCategory.UI, "AdminIncidents: load failed", {
  ...
  const openActions = (inc: Incident) => {
    Alert.alert("Resolver Ocorrência", inc.description, [
      { text: "Cancelar", style: "cancel" },
      {
        text: "Resolver",
        onPress: async () => {
          await api.adminResolveIncident(inc.id, "Resolvido via admin");
          load();
        },
      },
    ]);
  ...
  const typeColor = (type?: string) =>
    TYPE_COLOR[type?.toLowerCase() ?? ""] ?? "#64748B";
        ...
        {STATUSES.map((s) => (
          <Pressable
            ...
            style={[
              ...
              {
                backgroundColor:
                  filter === s ? BrandColors.primary : theme.cardBackground,
                borderColor: filter === s ? BrandColors.primary : theme.border,
              },
            ]}
            onPress={() => setFilter(s)}
          >
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item: inc }) => {
            ...
            const dt = new Date(inc.reported_at).toLocaleString("pt-PT", {
              day: "2-digit",
              month: "short",
              hour: "2-digit",
              minute: "2-digit",
                ...
                <View style={styles.cardRow}>
                  <View style={{ flex: 1 }}>
                    ...
                    >
                      ...
                      >
                        {inc.type_label ?? inc.type ?? "OCORRÊNCIA"}
                      </ThemedText>
                    </View>
                  ...
                  <View
                    style={[
                      ...
                      {
                        backgroundColor:
                          inc.status === "resolved" ? "#D1FAE5" : "#FEF3C7",
                      },
                    ]}
                  >
                    <ThemedText
                      ...
                      style={{
                        color:
                          inc.status === "resolved" ? "#065F46" : "#92400E",
                        fontWeight: "700",
                      }}
                    >
                      {inc.status}
