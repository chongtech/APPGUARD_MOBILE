...
export default function AdminVisitTypes() {
  ...
  const [reqRestaurant, setReqRestaurant] = useState(false);
  const [reqSport, setReqSport] = useState(false);

  const load = useCallback(async () => {
    setLoading(true);
    try {
      setItems(await api.adminGetAllVisitTypes());
    } catch (loadError) {
      logger.warn(LogCategory.UI, "AdminVisitTypes: load failed", {
  ...
  const openCreate = () => {
    ...
    setIconKey("");
    setReqService(false);
    setReqRestaurant(false);
    setReqSport(false);
    setModalOpen(true);
  };
  const openEdit = (v: VisitTypeConfig) => {
    setEditing(v);
    setName(v.name);
    setIconKey(v.icon_key ?? "");
    setReqService(v.requires_service_type);
    setReqRestaurant(v.requires_restaurant ?? false);
    setReqSport(v.requires_sport ?? false);
    setModalOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...
        requires_service_type: reqService,
        requires_restaurant: reqRestaurant,
        requires_sport: reqSport,
      };
  ...
  const handleDelete = (v: VisitTypeConfig) => {
    Alert.alert("Eliminar", `Eliminar "${v.name}"?`, [
      { text: "Cancelar", style: "cancel" },
      {
        ...
        style: "destructive",
        onPress: async () => {
          await api.adminDeleteVisitType(v.id);
          load();
        },
      },
    ]);
  ...
  const toggles = [
    ...
    {
      label: "Requer Restaurante",
      value: reqRestaurant,
      set: setReqRestaurant,
    },
          ...
          renderItem={({ item: v }) => (
            ...
            >
              <View style={styles.cardRow}>
                ...
                <Pressable onPress={() => openEdit(v)} style={styles.iconBtn}>
                  <Feather
                    name="edit-2"
                    size={16}
